import {
  ...
  Expand, Link2, Mic, Package, Pause, Play, RefreshCw,
...
const stepIcons = { ONION_HARVEST: Sprout, ONION_COLLECT: Sprout, BAGGING: Package, LOADING: Truck, WAREHOUSE_TRANSPORT: Warehouse, STACKING: Package };
...
export function OwnerHomeScreen({ go, session, setSession }: OwnerScreenProps) {
  ...
  useEffect(() => { let active = true; const load = async () => { setLoading(true); setError(''); try { const { items } = await api.listSessions(); if (active) setSession(items[0] ?? null); } catch (reason) { if (reason instanceof ApiError && reason.code === 'UNAUTHORIZED') { try { await api.createOwnerSession(); const { items } = await api.listSessions(); if (active) setSession(items[0] ?? null); return; } catch { /* show original error */ } } if (active) setError(errorText(reason)); } finally { if (active) setLoading(false); } }; void load(); return () => { active = false; }; }, [retry]);
...
export function OwnerTodayTeamScreen({ go }: OwnerScreenProps) {
  ...
  return <><PageHeading title="오늘 작업팀" description="QR 하나를 보여주면 근로자가 이름, 국적, 안내 언어를 직접 입력합니다." action={<ActionButton variant="secondary" onClick={() => go('owner-home')}><ArrowLeft className="h-5 w-5" />홈으로</ActionButton>} />
...
export function OwnerBriefScreen({ go, sessionId, workerLocale }: OwnerScreenProps) {
  ...
  if (error && !brief) return <><PageHeading title="브리핑을 열 수 없어요" description={error} /><ActionButton onClick={async () => { await api.createOwnerSession(); setRetry((value) => value + 1); }}>다시 연결</ActionButton></>;
