...
장소 생략 회귀: 초기 초안과 비위치 보완에서 `UNSPECIFIED`/null을 유지하고 장소 질문 없이 확정할 수 있어야 한다. 운반 목적지만 말했을 때 출발 장소로 옮겨 채우지 않는다. 실제 장소 후보 충돌과 알아들을 수 없는 명시적 장소는 차단한다. 구조화 `low`는 동일 모델의 `medium`과 양파·딸기, 수량·순서·상세 조건·금지·지원 외 작업·안전 차단을 비교한다. 측정값은 구조화 요청 시간이며 전체 STT/번역/TTS/DB 지연으로 주장하지 않는다.

## 상세 지시·사전·수량 회귀

농업 용어는 기존 DB 자료를 관련 문장의 glossary로 전달하는지, vi/ne가 섞이지 않는지, 용어 HIT를 문장 전체 공식 출처로 승격하지 않는지 검사한다. 수량 대조군은 20망/스무 망/이십 망 대 20만 개/이십만 개, 장소 번호, 복수 수량, 용기당 수량, 취소, 비수량 보완을 포함한다. STT는 높은 확신의 `스무만`도 독립 재전사를 거쳐 `스무 망`으로 확인되거나 `AUDIO_UNCLEAR`로 닫혀야 하며 200000으로 진행하면 실패다. 정상 `이십만 개`는 추가 재전사 없이 보존한다. 의미 충돌은 unknown/blocking으로 남기고 원문을 변경하지 않는다.
...
팀 배정 회귀는 렌더 결과만 확인하지 않고 Promise 완료 순서를 제어한다. 마지막 성공 작업 목록 뒤 polling 실패에서도 선택·버튼이 유지되어야 한다. 첫 배정 요청이 commit 전에 timeout되고 두 번째 동일 요청이 성공하는 순서, 첫 요청이 commit 후 응답만 유실되는 순서, 두 요청과 최종 roster 확인이 모두 실패하는 순서를 각각 검증한다. 성공 상태는 mutation 응답 또는 roster의 정확한 member/session 연결이 있을 때만, 실패는 재시도와 최종 확인이 끝난 뒤에만 표시한다.
...
단계 화면에는 전체 듣기 하나만 표시한다. 전체 듣기는 표시 텍스트와 hash가 맞는 서버 TTS를 사용한다. 저장 음성 hash 불일치·검증 불가·재생 실패는 동일한 전체 텍스트의 기기 음성으로 전환해야 한다. 기기 음성 미지원은 글 안내·해당 언어 오류·재시도를 유지하고 화면 진입을 막지 않아야 하며, 이동·버전 변경 후 이전 음성이 다시 시작하지 않아야 한다. 작업별 변경 알림은 해당 작업의 확인 상태와 일치해야 한다. 원본 종횡비가 다른 승인 영상을 320/390/1024px에서 잘림·가로 스크롤 없이 표시한다. 양파 다듬기와 운반 단계가 함께 있으면 각 task code에 맞는 서로 다른 `APPROVED`·`LOW` 영상이 신규 package에 들어가야 한다. 브라우저 mock 검증은 실제 STT·번역 의미 품질과 DB transaction 검증을 대신하지 않는다.
