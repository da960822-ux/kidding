...
const briefingSpeechText = (briefing: V2WorkerBriefing) => {
  ...
  return [context.location_display, quantity, context.deadline, ...context.safety, ...steps.map((step) => `${step.title} ${step.description}`), context.notes].filter((text) => typeof text === 'string' && text.trim()).join('\n');
};

export function BriefingNotes({ briefing }: { briefing: V2WorkerBriefing }) {
  if (!briefing.context.notes) return null;
  return <div className="mt-5"><Callout><strong>{briefing.language_code === 'vi' ? 'Lưu ý' : 'ध्यान दिनुहोस्'}</strong><p className="mt-2 whitespace-pre-line text-lg font-extrabold leading-8">{briefing.context.notes}</p></Callout></div>;
}
...
const extractToken = (value: string, kind: 'team' | 'work') => value.trim().match(new RegExp(`/${kind === 'team' ? 'team' : 'w'}/([^/?#]+)`))?.[1] ?? (/^[A-Za-z0-9_-]{32,}$/.test(value.trim()) ? value.trim() : '');
...
function WorkerLatest({ assignment, assignments, selectAssignment, go }: { assignment: V2WorkerBriefing; assignments: V2WorkerBriefing[]; selectAssignment: (sessionId: string) => void; go: ScreenProps['go'] }) {
  ...
  return <>
    ...
    <section className="mb-5 rounded-2xl bg-[#E5F0E0] p-6 sm:p-8"><StatusBadge>{locale === 'vi' ? 'Tiếng Việt' : 'नेपाली'}</StatusBadge><h1 className="mt-4 text-2xl font-black sm:text-3xl">{first?.title ?? t.today}</h1><BriefingNotes briefing={assignment} />{first ? <ActionButton className="mt-5 w-full" onClick={() => go('worker-step')}>{t.startFirst}<ArrowRight className="h-5 w-5" /></ActionButton> : <Callout>{t.invalid}</Callout>}<div className="mt-5 grid gap-1 sm:grid-cols-2"><FactRow label={t.location} value={assignment.context.location_display} /><FactRow label={t.amount} value={quantityText(assignment.context.quantity)} /><FactRow label={t.deadline} value={assignment.context.deadline ?? '—'} last /></div>{assignment.context.safety.length > 0 && <div className="mt-5"><Callout><strong>{t.safety}</strong><p>{assignment.context.safety.join(' · ')}</p></Callout><SafetySources briefing={assignment} /></div>}</section>
    <details className="rounded-3xl border border-deep/10 bg-white px-5 sm:px-6"><summary className="flex min-h-14 cursor-pointer items-center py-3 text-lg font-black text-deep focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25">{t.more}</summary><div className="border-t border-deep/10 py-5"><PanelHeader title={t.today} aside={<StatusBadge tone="blue">{assignment.steps.length}</StatusBadge>} /><ol className="grid gap-3">{assignment.steps.map((step) => <li key={step.sequence} className="flex items-center gap-4 rounded-2xl bg-[#F5F8F3] p-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-deep font-black text-white">{step.sequence}</span><strong className="text-lg leading-7">{step.title}</strong></li>)}</ol><div className="mt-5"><BriefingAudioControls briefing={assignment} /></div></div></details>
  </>;
}
...
export function WorkerScreenRouter({ screen, go, token, locale, entryLocale, setLocale }: WorkerScreenProps) {
  ...
  const [selectedSessionId, setSelectedSessionId] = useState<string | null>(null); const [errorCode, setErrorCode] = useState(''); const [refresh, setRefresh] = useState(0); const [updatedSessionIds, setUpdatedSessionIds] = useState<string[]>([]); const [loaded, setLoaded] = useState(false);
  ...
  useEffect(() => { setAssignments([]); setReceipts([]); setSelectedSessionId(null); setLoaded(false); setUpdatedSessionIds([]); setErrorCode(''); setAckError(''); knownVersions.current.clear(); }, [token]);
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const changedIds = result.assignments.filter((item) => knownVersions.current.has(item.session_id) && item.version > knownVersions.current.get(item.session_id)!).map((item) => item.session_id);
        if (changedIds.length) setUpdatedSessionIds((ids) => [...new Set([...ids, ...changedIds])]);
  ...
  const updated = updatedSessionIds.includes(assignment.session_id) && (!isTeamMember || !acknowledged);
  const otherChanges = pending.filter((item) => item.session_id !== assignment.session_id && updatedSessionIds.includes(item.session_id));
