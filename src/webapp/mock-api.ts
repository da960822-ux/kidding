...
const v2Briefing = (session: OwnerWorkSession, locale: WorkerLocale, task_family: 'ONION' | 'STRAWBERRY' = 'ONION'): V2WorkerBriefing => {
  const quantity = session.version.state.quantity;
  const task_code: V2TaskCode = task_family === 'ONION' ? 'ONION_HARVEST' : 'STRAWBERRY_HARVEST';
  const title = task_family === 'ONION' ? (locale === 'vi' ? 'Thu hoạch hành' : 'प्याज काट्नुहोस्') : (locale === 'vi' ? 'Thu hoạch dâu tây' : 'स्ट्रबेरी टिप्नुहोस्');
  return { session_id: session.session_id, version: session.current_version, contract_version: 'worker-briefing-v2', ontology_version: 'ontology-v2', language_code: locale, context: { task_family, location_display: task_family === 'ONION' ? 'Ruộng số 1' : 'Ruộng dâu số 2', quantity: typeof quantity === 'object' ? quantity : null, deadline: '11:00', notes: null }, badges: ['TEXT_TTS_FALLBACK', 'DEMO_FALLBACK'], steps: [{ sequence: 1, task_code, title, description: title, delivery_mode: 'TEXT_TTS' }], source_detail: [{ step_sequence: 1, segment: 'ACTION', source: 'AI_TRANSLATION', guide_lookup: 'MISS', verified: false, source_page: null, source_url: null, license: null }], tts: { status: 'FALLBACK', text_hash: hash(`${task_family}-${session.current_version}-${typeof quantity === 'object' && quantity ? quantity.value : 'unknown'}`), audio_url: null }, video: [] };
};
...
export function createMockApi(ApiError: ApiErrorConstructor) {
  return {
    ...
    getTodayTeam: (): Promise<TodayWorkTeam> => { const team = loadTeam(); return team ? wait(team) : Promise.reject(new ApiError(404, 'NOT_FOUND', '오늘 작업팀이 아직 없습니다.')); },
    joinTodayTeam: (token: string, input: { display_name: string; language_code: WorkerLocale }): Promise<TeamMember> => { const team = loadTeam(); if (!team?.join_url?.endsWith(`/team/${token}`)) return Promise.reject(new ApiError(404, 'ACCESS_DENIED', '참여 코드를 확인해주세요.')); if (new Date(team.expires_at) <= new Date()) return Promise.reject(new ApiError(410, 'LINK_EXPIRED', '참여 시간이 끝났습니다.')); const member = { member_id: crypto.randomUUID(), ...input, joined_at: new Date().toISOString(), assignment_session_ids: [] }; saveTeam({ ...team, members: [...team.members, member] }); window.localStorage.setItem(TEAM_MEMBER_KEY, member.member_id); return wait(member); },
