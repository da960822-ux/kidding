...
import type { IssuedWorkerLink, OwnerSession, OwnerWorkSession, PersonalInstructionTarget, WorkDraft } from './contracts';
...
const personalInstructionTargetStorageKey = 'batmeori-personal-instruction-target';

function savedPersonalInstructionTarget(): PersonalInstructionTarget | null {
  try {
    const value = JSON.parse(window.sessionStorage.getItem(personalInstructionTargetStorageKey) ?? 'null') as Partial<PersonalInstructionTarget> | null;
    return value && typeof value.member_id === 'string' && typeof value.display_name === 'string' && (value.language_code === 'vi' || value.language_code === 'ne') ? value as PersonalInstructionTarget : null;
  } catch { return null; }
}
const roleCopy = {
...
export function WebApp({ initialLocale }: { initialLocale: Locale }) {
  ...
  const [personalInstructionTarget, setPersonalInstructionTargetState] = useState<PersonalInstructionTarget | null>(savedPersonalInstructionTarget); const [personalAssignmentNotice, setPersonalAssignmentNotice] = useState('');
  ...
  const setPersonalInstructionTarget = (next: PersonalInstructionTarget | null) => { setPersonalInstructionTargetState(next); if (next) window.sessionStorage.setItem(personalInstructionTargetStorageKey, JSON.stringify(next)); else window.sessionStorage.removeItem(personalInstructionTargetStorageKey); };
  const advanceOwnerAuthGeneration = () => { ownerAuthGeneration.current += 1; setOwnerAuthGeneration(ownerAuthGeneration.current); };
  const clearOwnerState = (resetRoute = false, preserveDraft = false) => { advanceOwnerAuthGeneration(); setOwnerSession(null); setSession(null); setPersonalInstructionTarget(null); setPersonalAssignmentNotice(''); if (preserveDraft) { setDraftState(null); setDraftRecoveryError(false); } else setDraft(null); setDraftReadFailed(false); setDraftAudio(null); setIssuedWorkerLink(null); if (resetRoute) { authenticatedOwnerScope.current = null; returnScreen.current = 'owner-home'; sessionIdRef.current = null; setSessionId(null); } };
  ...
  const startPersonalInstruction = (target: PersonalInstructionTarget) => { setDraft(null); setDraftAudio(null); setPersonalAssignmentNotice(''); setPersonalInstructionTarget(target); go('owner-record'); };
  const finishPersonalInstruction = (notice: string) => { setPersonalInstructionTarget(null); setPersonalAssignmentNotice(notice); };
  ...
  useEffect(() => { if (screen !== 'owner-record' && screen !== 'owner-review') setPersonalInstructionTarget(null); }, [screen]);
