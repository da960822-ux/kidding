...
export function OwnerHomeScreen({ go, session, setSession }: OwnerScreenProps) {
  ...
  return <><PageHeading title="오늘 어떤 작업을 시킬까요?" description="평소 말투 그대로 말씀하세요. AI가 추측하지 않고 정리합니다." />
    ...
    <h2 className="mb-4 text-xl font-black">최근 작업</h2>{loading ? <div className="h-36 animate-pulse rounded-2xl bg-sage/40" aria-label="작업을 불러오는 중" /> : error ? <div><Callout>{error}</Callout><ActionButton className="mt-4" onClick={() => setRetry((value) => value + 1)}>다시 연결</ActionButton></div> : session ? <button type="button" onClick={() => go('owner-current')} className="w-full rounded-2xl bg-white p-5 text-left shadow-soft focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25"><div className="flex justify-between gap-3"><strong className="text-xl">{session.version.state.steps.map((step) => step.title_ko).join(' · ') || '작업 미지정'}</strong><StatusBadge>최신</StatusBadge></div><p className="mt-2 font-bold text-muted">{session.version.state.location_display} · {quantityText(session.version.state.quantity)}</p></button> : <div className="rounded-2xl border border-dashed border-deep/20 bg-white p-8 text-center"><Sprout className="mx-auto h-10 w-10 text-primary/50" /><h3 className="mt-4 text-xl font-black">진행 중인 작업이 없어요</h3></div>}
  </>;
}
...
export function OwnerReviewScreen({ go, draft, setDraft, draftAudio, setDraftAudio, setSession, workerLocale, setWorkerLocale, setIssuedWorkerLink }: OwnerScreenProps) {
  ...
  const confirm = async () => {
    ...
    try { const next = await api.confirmDraft(draft.draft_id, needsOverride ? 'PUBLISH_AS_IS' : 'CONFIRM', needsOverride ? reason || undefined : undefined); setSession(next.work_session); setIssuedWorkerLink(next.issued_worker_link); setDraftAudio(null); go('owner-storyboard'); }
  ...
  return <><PageHeading title="제가 이렇게 이해했어요" description="AI는 추측하지 않습니다. 미확정 내용은 농장주가 결정해주세요." />
    <Panel className="mx-auto max-w-4xl"><div className="text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-sage text-deep"><Sparkles className="h-8 w-8" /></span><p className="mx-auto mt-6 max-w-3xl text-2xl font-black leading-10 sm:text-3xl">{draft.summary_ko}</p></div>
      <div className="mt-6 rounded-2xl bg-[#F2F6F0] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><span className="block font-bold text-muted">인식한 수량</span><strong className="mt-1 block text-3xl text-deep">{quantityText(draft.state.quantity)}</strong></div><ActionButton variant="secondary" onClick={() => { setCorrectingQuantity((value) => !value); setCorrectionStatus(''); }}><Mic className="h-5 w-5" />{correctingQuantity ? '수량 수정 닫기' : '수량 다시 말하기'}</ActionButton></div>{correctingQuantity && <div className="mt-4"><VoiceRecorder helper="숫자와 단위만 짧게 다시 말해주세요." submitLabel="수량 다시 확인" onSubmit={correctQuantity} /></div>}{correctionStatus && <p role="status" className="mt-4 font-bold text-deep">{correctionStatus}</p>}</div>
      ...
      <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><ActionButton variant="secondary" disabled={!draftAudio} onClick={playOriginalAudio}><Volume2 className="h-5 w-5" />원음 다시 듣기</ActionButton><ActionButton disabled={blocked || loading || (needsOverride && !reason)} onClick={confirm}><Check className="h-5 w-5" />{loading ? '확정하고 있어요…' : needsOverride ? '이대로 전달' : '맞아, 확정하기'}</ActionButton><ActionButton variant="quiet" onClick={() => { setDraftAudio(null); go('owner-record'); }}>처음부터 다시</ActionButton></div>
    </Panel>
...
export function OwnerBriefScreen({ go, sessionId, workerLocale }: OwnerScreenProps) {
  const [brief, setBrief] = useState<Briefing | null>(null); const [error, setError] = useState(''); const [index, setIndex] = useState(0); const [updated, setUpdated] = useState(false); const [retry, setRetry] = useState(0); const audio = useRef<HTMLAudioElement | null>(null); const videoElement = useRef<HTMLVideoElement | null>(null); const playAfterMove = useRef(false);
  useEffect(() => {
    if (!sessionId) return;
