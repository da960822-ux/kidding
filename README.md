...
**출력 형식이 맞는 것과 작업 의미가 맞는 것을 따로 평가했습니다.** 아래는 팀이 제공한 30건 비교 결과입니다.

| 조건 | Schema | Semantic |
|---|---:|---:|
| 기존 Terra prompt-only | 30/30 | 25/30 (83.33%) |
| Terra reference prompt-only | 30/30 | 26/30 (86.67%) |
| Sol reference prompt-only | 30/30 | 21/30 (70.00%) |
| **Terra reference + 구조 정규화** | **30/30** | **30/30 (100.00%)** |

이 비교에서는 모든 조건이 Schema 검사를 통과했지만 Semantic 결과는 달랐습니다. **Terra reference + 구조 정규화는 기존 Terra prompt-only 대비 의미 검사 통과가 5건 늘어 16.67%p 상승**했습니다. reference prompt-only 대비로는 4건 증가했습니다. 해당 30건에서 관찰한 결과이며, 전체 사투리·음성 인식 정확도가 100%라는 뜻은 아닙니다.

<sub>출처: 팀 제공 평가 요약. 이 30건 비교의 데이터셋·채점 기준·실행 로그는 현재 저장소에 연결되지 않았습니다. 아래의 원본 로그가 있는 23건 × 3회 사전 on/off 실험과는 별도 결과입니다.</sub>
