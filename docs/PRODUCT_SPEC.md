...
작업 장소는 선택 정보다. 장소를 말하지 않은 양파·딸기 지시는 `장소 미지정`으로 유지하고, 그 생략만으로 추가 녹음이나 장소 입력을 요구하지 않는다. 실제 장소 후보 충돌, 불명확한 발화, 수량·작업·안전 차단은 기존대로 확인한다.
...
- 신규 양파 운반 지시는 `ONION_TRANSPORT`로 유지하고, 사람 검수된 `APPROVED`·`LOW` 운반 영상이 있으면 다른 단계와 같은 규칙으로 전달한다. 기존 확정 version은 변경하지 않는다.
...
농장주 팀 화면은 마지막으로 성공한 작업 목록과 roster를 재조회 상태와 분리한다. 백그라운드 재조회가 실패하거나 진행 중이어도 이미 확인한 작업의 선택·배정 버튼을 막거나 목록을 지우지 않는다. 배정 응답이 유실되거나 5xx이면 같은 근로자·작업의 멱등 배정을 한 번 재시도하고, 그래도 결과가 불명확할 때 roster에서 활성 배정을 확인한 뒤에만 실패를 표시한다. 4xx 계약 오류는 즉시 표시한다.
...
`전체 지시 듣기`는 저장된 WorkerBriefing의 표시 내용으로 조립한 전체 텍스트와 `tts.text_hash`가 일치할 때 저장된 TTS를 재생한다. 과거의 일부 내용만 읽는 음성처럼 hash가 다르거나 검증할 수 없거나 재생에 실패하면 같은 전체 텍스트를 기기 음성으로 읽는다. 단계 화면은 이 전체 듣기 하나만 제공해 음성 범위를 혼동시키지 않는다. 기기 음성도 지원되지 않으면 선택 언어로 글 안내와 재시도를 제공한다. 화면·단계·버전·언어가 바뀌면 이전 재생을 정리한다. 근로자 변경 알림은 바뀐 작업을 특정하고 해당 버전의 명시적 확인 뒤 해제한다.

근로자 단계 화면과 농장주 브리핑의 검수 영상은 원본 프레임 전체가 보이도록 고유 종횡비를 유지한다. 모바일·데스크톱에서 고정 `aspect-video`나 `object-cover`로 자르지 않는다.
...
- P0 영상은 AI 사전 생성·사람 검수 자산만 사용한다. `ONION_TRANSPORT` 영상은 차량 운전·차량 이동이 없는 수작업 운반 자산만 `LOW`로 게시한다.
