...
import type { Briefing, IssuedWorkerLink, OverrideReason, OwnerWorkSession, QuantityChangePreview, TodayWorkTeam, WorkDraft, WorkStep } from './contracts';
...
function StorySteps({ steps }: { steps: WorkStep[] }) {
  return <ol>{steps.map((step) => {
    const Icon = step.task_code ? stepIcons[step.task_code] : AlertTriangle;
...
export function OwnerReviewScreen({ go, draft, setDraft, setSession, workerLocale, setWorkerLocale, setIssuedWorkerLink }: OwnerScreenProps) {
  const [reason, setReason] = useState<OverrideReason | ''>(''); const [deliveryMode, setDeliveryMode] = useState<'CO_PRESENT' | 'REMOTE'>('CO_PRESENT'); const [loading, setLoading] = useState(false); const [error, setError] = useState('');
  ...
  const confirm = async () => {
    ...
    try { const next = await api.confirmDraft(draft.draft_id, needsOverride ? 'PUBLISH_AS_IS' : 'CONFIRM', deliveryMode, workerLocale, needsOverride ? reason || undefined : undefined); setSession(next.work_session); setIssuedWorkerLink(next.issued_worker_link); go('owner-storyboard'); }
  ...
  return <><PageHeading title="제가 이렇게 이해했어요" description="AI는 추측하지 않습니다. 미확정 내용은 농장주가 결정해주세요." />
    <Panel className="mx-auto max-w-4xl"><div className="text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-sage text-deep"><Sparkles className="h-8 w-8" /></span><p className="mx-auto mt-6 max-w-3xl text-2xl font-black leading-10 sm:text-3xl">{draft.summary_ko}</p></div>
      ...
      {!blocked && <div className="mt-6 grid gap-4 rounded-2xl bg-[#F2F6F0] p-4"><strong>전달 방법과 언어</strong><div className="grid gap-2 sm:grid-cols-2">{([['CO_PRESENT', '현장에서 같이 보기'], ['REMOTE', '링크로 보내기']] as const).map(([value, label]) => <button key={value} type="button" onClick={() => setDeliveryMode(value)} className={`min-h-14 rounded-xl border px-4 font-black ${deliveryMode === value ? 'border-deep bg-deep text-white' : 'border-deep/15 bg-white'}`}>{label}</button>)}</div><div className="grid grid-cols-2 gap-2">{(['vi', 'ne'] as const).map((locale) => <button key={locale} type="button" onClick={() => setWorkerLocale(locale)} className={`min-h-12 rounded-xl border px-3 font-black ${workerLocale === locale ? 'border-deep bg-sage' : 'border-deep/15 bg-white'}`}>{locale === 'vi' ? 'Tiếng Việt' : 'नेपाली'}</button>)}</div></div>}
...
export function OwnerStoryboardScreen({ go, session, workerLocale, issuedWorkerLink, setIssuedWorkerLink }: OwnerScreenProps) {
  const [issued, setIssued] = useState<IssuedWorkerLink | null>(issuedWorkerLink); const [copyStatus, setCopyStatus] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  ...
  return <><PageHeading title="작업 스토리보드" description="확정 때 선택한 전달 방식과 언어로 보냅니다." /><div className="mb-5 grid gap-3 rounded-2xl bg-[#EAF2E6] p-5 sm:grid-cols-3"><FactRow label="장소" value={session.version.state.location_display} last /><FactRow label="수량" value={quantityText(session.version.state.quantity)} last /><FactRow label="버전" value={`v${session.current_version}`} last /></div><div className="grid gap-5 xl:grid-cols-[1fr_350px]"><Panel className="order-2 xl:order-1"><PanelHeader title="양파 작업 단계" aside={<StatusBadge>{session.version.state.steps.length}단계</StatusBadge>} /><StorySteps steps={session.version.state.steps} /></Panel><div className="order-1 grid content-start gap-5 xl:order-2">{issued ? <Panel><PanelHeader title="24시간 링크" /><Callout safe><strong>링크는 지금 한 번만 표시됩니다.</strong><p className="break-all text-base">{issued.issued_worker_link.url}</p><p className="text-base">만료: {new Date(issued.issued_worker_link.expires_at).toLocaleString('ko-KR')}</p></Callout><div className="mt-3 grid gap-2"><ActionButton onClick={copy}><Copy className="h-5 w-5" />링크 복사</ActionButton><ActionButton variant="secondary" disabled={loading} onClick={reissue}>{loading ? '새 링크 발급 중…' : '새 링크 발급'}</ActionButton><a className="inline-flex min-h-12 items-center justify-center rounded-2xl border border-primary/35 font-extrabold text-deep" href={issued.issued_worker_link.url}>작업자 화면 열기</a></div>{copyStatus && <p role="status" className="mt-3 text-base font-extrabold text-deep">{copyStatus}</p>}{error && <p role="alert" className="mt-3 text-base font-bold text-[#8A302B]">{error}</p>}</Panel> : <Panel><PanelHeader title="현장에서 같이 보기" /><p className="text-base font-bold text-muted">이 휴대폰에서 선택한 언어의 최신 작업을 보여줍니다.</p><ActionButton className="mt-4 w-full" onClick={() => go('owner-brief')}><Play className="h-5 w-5" />브리핑 시작</ActionButton></Panel>}</div></div></>;
}
