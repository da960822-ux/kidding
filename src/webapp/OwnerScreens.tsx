...
const workerLanguageNames: Record<WorkerLocale, string> = { vi: 'Tiếng Việt', ne: 'नेपाली', km: 'ភាសាខ្មែរ' };
const briefingCopy: Record<WorkerLocale, { updated: string; stale: string; retry: string; title: string; demo: string; confirm: string; fallback: string; prev: string; listen: string; next: string; location: string; quantity: string; fullscreen: string; speech: string }> = {
  vi: { updated: 'Có hướng dẫn mới. Hãy xem lại từ bước đầu tiên.', stale: 'Không thể kiểm tra hướng dẫn mới nhất.', retry: 'Thử lại', title: 'Hướng dẫn công việc', demo: 'NỘI DUNG DEMO', confirm: 'Cần xác nhận', fallback: 'Không có video đã kiểm duyệt. Hãy đọc hoặc nghe hướng dẫn.', prev: 'Trước', listen: 'Nghe', next: 'Tiếp', location: 'Địa điểm', quantity: 'Số lượng', fullscreen: 'Toàn màn hình', speech: 'vi-VN' },
  ne: { updated: 'नयाँ निर्देशन आएको छ। पहिलो चरणदेखि फेरि हेर्नुहोस्।', stale: 'नयाँ निर्देशन जाँच्न सकिएन।', retry: 'फेरि प्रयास', title: 'काम निर्देशन', demo: 'डेमो सामग्री', confirm: 'पुष्टि आवश्यक', fallback: 'जाँच गरिएको भिडियो छैन। निर्देशन पढ्नुहोस् वा सुन्नुहोस्।', prev: 'अघिल्लो', listen: 'सुन्नुहोस्', next: 'अर्को', location: 'स्थान', quantity: 'परिमाण', fullscreen: 'पूरा स्क्रिन', speech: 'ne-NP' },
  km: { updated: 'មានសេចក្តីណែនាំថ្មី។ សូមមើលឡើងវិញចាប់ពីជំហានទីមួយ។', stale: 'មិនអាចពិនិត្យសេចក្តីណែនាំថ្មីបំផុតបានទេ។', retry: 'ព្យាយាមម្តងទៀត', title: 'សេចក្តីណែនាំការងារ', demo: 'ខ្លឹមសារសាកល្បង', confirm: 'ត្រូវការបញ្ជាក់', fallback: 'មិនមានវីដេអូដែលបានត្រួតពិនិត្យទេ។ សូមអាន ឬស្តាប់សេចក្តីណែនាំ។', prev: 'មុន', listen: 'ស្តាប់', next: 'បន្ទាប់', location: 'ទីតាំង', quantity: 'បរិមាណ', fullscreen: 'ពេញអេក្រង់', speech: 'km-KH' },
};
...
export function OwnerTodayTeamScreen({ go }: OwnerScreenProps) {
  ...
  const showTeam = (next: TodayWorkTeam) => setTeam((current) => current
    ? { ...current, status: next.status, members: next.team_id === current.team_id ? next.members : current.members }
    : next);
...
export function OwnerBriefScreen({ go, sessionId, workerLocale }: OwnerScreenProps) {
  ...
  const step = brief.steps[Math.min(index, brief.steps.length - 1)]; const t = briefingCopy[workerLocale]; const video = step.video?.review_status === 'APPROVED' && step.video.safety_level === 'LOW' ? step.video : null;
  const browserSpeech = () => { const speech = new SpeechSynthesisUtterance(step.description); speech.lang = t.speech; window.speechSynthesis.cancel(); window.speechSynthesis.speak(speech); };
  ...
  return <>{updated && <div role="status" aria-live="assertive" className="mb-5 rounded-2xl bg-[#173F24] p-5 text-lg font-black text-white">{t.updated}</div>}{error && <div role="alert" className="mb-5 rounded-2xl bg-[#FFF0BF] p-5 text-base font-extrabold text-[#654B16]">{t.stale}<ActionButton className="ml-3" variant="secondary" onClick={() => setRetry((value) => value + 1)}>{t.retry}</ActionButton></div>}<PageHeading title={t.title} description={`v${brief.version} · ${index + 1}/${brief.steps.length}`} action={<ActionButton variant="secondary" onClick={() => go('owner-storyboard')}><ArrowLeft className="h-5 w-5" />돌아가기</ActionButton>} />{brief.badge_codes.map((badge) => <StatusBadge key={badge} tone="yellow">{badge === 'DEMO_FALLBACK' ? t.demo : t.confirm}</StatusBadge>)}<div className="mt-5 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]"><section>{video ? <video key={video.video_url} controls playsInline className="aspect-video w-full rounded-2xl bg-deep object-cover"><source src={video.video_url} /><track kind="captions" src={`data:text/vtt;charset=utf-8,${encodeURIComponent(`WEBVTT\n\n00:00.000 --> 23:59.000\n${video.captions_text}`)}`} srcLang={workerLocale} label={workerLocale} default /></video> : <Callout safe><p className="text-lg">{t.fallback}</p></Callout>}<div className="mt-4 grid grid-cols-[auto_1fr_auto] gap-2"><ActionButton variant="secondary" disabled={index === 0} onClick={() => setIndex((value) => value - 1)}>{t.prev}</ActionButton><ActionButton onClick={play}><Volume2 className="h-5 w-5" />{t.listen}</ActionButton><ActionButton variant="secondary" disabled={index === brief.steps.length - 1} onClick={() => setIndex((value) => value + 1)}>{t.next}</ActionButton></div></section><Panel><PanelHeader title={step.title} aside={<StatusBadge>v{brief.version}</StatusBadge>} /><p className="text-xl font-bold leading-9">{step.description}</p><FactRow label={t.location} value={brief.context.location_display} /><FactRow label={t.quantity} value={brief.context.quantity_display} last /><ActionButton className="mt-6 w-full" variant="quiet" onClick={() => document.documentElement.requestFullscreen?.()}><Expand className="h-5 w-5" />{t.fullscreen}</ActionButton></Panel></div></>;
}
