...
- Product scope for the 1-night/2-day hackathon is onion only, Vietnamese (`vi`) and Nepali (`ne`) only, plus quantity changes. Keep this scope explicit; do not imply whole-crop or all-language support.
...
- Canonical task codes are `ONION_HARVEST`, `ONION_COLLECT`, `BAGGING`, `LOADING`, `WAREHOUSE_TRANSPORT`, and `STACKING`. Use uppercase snake case.
...
- P0 has no worker registration, seed worker, phone number, SMS, or worker login. Delivery is either owner-present briefing (`vi|ne`) or one anonymous language-specific 24-hour link.
