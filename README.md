...
- 배포 FE는 `VITE_API_BASE_URL`로 API를 지정한다. browser mock은 `VITE_USE_MOCK_API=true`일 때만 허용하며 production fallback이 아니다.
- BE는 `FRONTEND_ORIGINS`의 exact Origin과 `PUBLIC_WEB_BASE_URL`을 설정한다. 후자는 `DEMO_FALLBACK=1`이 아닌 환경에서 필수이며, owner mutation은 PIN cookie와 exact Origin으로 보호한다. 정적 CSRF header는 사용하지 않는다.
- `REMOTE` 발급 URL은 `${PUBLIC_WEB_BASE_URL}/w/{token}`이다. 이 browser route가 JSON assignment endpoint를 호출하며, 기존 v1 record는 reset·rewrite·자동 remap 없이 읽기 전용으로 보존한다.
