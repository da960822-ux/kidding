...
- Product scope for the 1-night/2-day hackathon is onion and strawberry only, Vietnamese (`vi`) and Nepali (`ne`) only, plus quantity changes. Keep this two-crop scope explicit; do not imply whole-crop or all-language support.
- P0 videos are AI-generated in advance and human-reviewed. Store provenance and `review_status`; assets with `safety_level: HIGH` are never publishable. Do not describe direct filming as an implementation path.
...
- Canonical task codes are `ONION_HARVEST`, `ONION_TRIMMING`, `ONION_SORTING`, `ONION_TRANSPORT`, `STRAWBERRY_HARVEST`, `STRAWBERRY_SORTING`, `STRAWBERRY_INSPECTION`, and `STRAWBERRY_PACKING`. A step code must belong to its `task_family`; use uppercase snake case.
- Owner mutations require a signed workspace/team cookie and exact Origin; explicitly public start/PIN endpoints issue that cookie. New owners start without farm code/name/PIN input. First publish activates one temporary team's management PIN and QR for exactly 24 hours; adding or changing work never extends expiry. Other devices resume with the owner management link and PIN. Preserve legacy farm authentication only as a compatibility path. Worker links expire 24 hours after issue and always resolve the latest `PUBLISHED` WorkSession version. External access errors are generalized; expired links receive reissue guidance.
- P0 has no persistent worker registration, seed worker, phone number, SMS, or worker login. A `TodayWorkTeam` is a 24-hour anonymous QR roster: a worker supplies only a display name and `vi|ne`, then receives a signed browser session for that temporary team. It may be assigned one or more latest `PUBLISHED` WorkSessions. Delivery also retains owner-present briefing (`vi|ne`) and one anonymous language-specific 24-hour link.
- TeamAssignment acknowledgement records the worker's explicit confirmation of a displayed version, never task completion or automatic read acknowledgement. New or changed assignments remain unconfirmed until that action. Foreground polling alerts do not imply closed-browser OS push delivery.
