...
export function OwnerTodayTeamScreen({ go, session, ownerSession }: OwnerScreenProps) {
  ...
  const [team, setTeam] = useState<TodayWorkTeam | null>(null); const [sessions, setSessions] = useState<OwnerWorkSession[]>([]); const [selected, setSelected] = useState<Record<string, string>>({}); const [assigning, setAssigning] = useState<string | null>(null); const [assignmentStatus, setAssignmentStatus] = useState(''); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [copyStatus, setCopyStatus] = useState(''); const [confirmRotate, setConfirmRotate] = useState(false); const teamGeneration = useRef(0); const rotating = useRef(false); const teamRef = useRef<TodayWorkTeam | null>(null);
  const showTeam = (next: TodayWorkTeam) => { teamRef.current = next; setTeam(next); };
  useEffect(() => {
    let active = true; let inFlight = false;
    const refresh = async () => {
      if (inFlight || rotating.current) return;
  ...
  const assign = async (memberId: string, workSessionId: string) => {
    ...
    teamGeneration.current += 1; setAssigning(memberId); setAssignmentStatus(''); setError('');
    try {
      ...
      if (reason instanceof ApiError && reason.status < 500) setError(errorText(reason));
      else {
        setAssignmentStatus('배정 결과를 확인하고 있어요…');
        try {
          const next = await api.getTodayTeam(); showTeam(next);
          ...
          else { setAssignmentStatus(''); setError('배정 결과를 확인하지 못했어요. 잠시 후 작업팀을 다시 확인해주세요.'); }
        } catch { setAssignmentStatus(''); setError('배정 결과를 확인하지 못했어요. 잠시 후 작업팀을 다시 확인해주세요.'); }
      }
