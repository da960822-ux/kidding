...
function routeState() {
  ...
  else if (managementMatch) screen = 'owner-team';
  ...
  return { managementId: managementMatch?.[1] ?? null, screen: requested && appScreens.includes(requested) ? requested : screen, sessionId: ownerMatch?.[1] ?? null, token: teamMemberPath ? '__team_member__' : workerMatch?.[1] ?? null };
}
...
function OwnerLoginScreen({ login, cancel, startNew, hasTeam }: { login: (pin: string) => Promise<void>; cancel: () => void; startNew: () => Promise<void>; hasTeam: boolean }) {
  ...
  return <div className="mx-auto max-w-xl py-6 sm:py-14"><button type="button" onClick={cancel} className="mb-5 inline-flex min-h-11 items-center gap-2 rounded-xl px-3 font-extrabold text-deep"><ArrowLeft className="h-5 w-5" />처음으로</button><Panel><KeyRound className="h-10 w-10 text-deep" /><h1 className="mt-6 text-3xl font-black">{hasTeam ? '팀 관리 다시 열기' : '작업팀 연결이 필요해요'}</h1><p className="mt-3 font-bold leading-7 text-muted">{hasTeam ? '처음 작업을 확정할 때 받은 6자리 PIN을 입력해주세요. 팀은 첫 확정부터 24시간 동안 유지됩니다.' : '저장한 관리 링크로 팀을 다시 열어주세요. 만료된 팀은 새 팀으로 시작할 수 있습니다.'}</p>{hasTeam && <form className="mt-6 grid gap-4" onSubmit={submit}><label htmlFor="owner-pin" className="grid gap-2 font-extrabold">PIN<input id="owner-pin" type="password" value={pin} onChange={(event) => setPin(event.target.value)} inputMode="numeric" pattern="[0-9]{6}" minLength={6} maxLength={6} required autoComplete="current-password" className="min-h-14 rounded-xl border border-deep/20 px-4 text-lg focus:ring-4 focus:ring-primary/20" /></label><ActionButton type="submit" disabled={loading}>{loading ? '연결 중…' : '이 팀 열기'}</ActionButton></form>}{error && <p role="alert" className="mt-4 rounded-xl bg-[#FDE7E4] p-4 font-bold text-[#8A302B]">{error}</p>}<ActionButton variant="secondary" className="mt-5 w-full" disabled={loading} onClick={async () => { setLoading(true); try { await startNew(); } catch { setError('새 팀을 시작하지 못했습니다. 다시 시도해주세요.'); } finally { setLoading(false); } }}>새 팀으로 시작</ActionButton></Panel></div>;
}
...
export function WebApp({ initialLocale }: { initialLocale: Locale }) {
  ...
  const clearOwnerState = (resetRoute = false) => { advanceOwnerAuthGeneration(); setOwnerSession(null); setSession(null); setDraft(null); setDraftAudio(null); setIssuedWorkerLink(null); if (resetRoute) { authenticatedOwnerScope.current = null; returnScreen.current = 'owner-home'; sessionIdRef.current = null; setSessionId(null); } };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        try { next = await api.getOwnerSession(); if (screen === 'owner-record' && !next.team) next = await api.startOwnerSession(); }
        catch (reason) {
  ...
  useEffect(() => { const pop = () => { const next = routeState(); setScreen(next.screen); sessionIdRef.current = next.sessionId; setSessionId(next.sessionId); setToken(next.token); setManagementId(next.managementId); if (next.managementId && next.managementId !== ownerSession?.team?.team_id) { clearOwnerState(); setOwnerAuth('idle'); } }; window.addEventListener('popstate', pop); return () => window.removeEventListener('popstate', pop); }, [ownerSession?.team?.team_id]);
  ...
  const startNew = async () => {
    ...
    const next = await api.startOwnerSession(); setOwnerSession(next); setOwnerAuth('authenticated'); go('owner-record');
  };
