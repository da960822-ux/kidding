...
export function OwnerTodayTeamScreen({ go, session, ownerSession }: OwnerScreenProps) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      catch (reason) { if (active && generation === teamGeneration.current && (!(reason instanceof ApiError) || reason.code !== 'NOT_FOUND' && reason.status !== 409)) setError(errorText(reason)); }
      finally { inFlight = false; if (active) setLoading(false); }
    };
...
export function OwnerReviewScreen({ go, draft, setDraft, draftAudio, setDraftAudio, draftRecoveryError, setSession, setIssuedWorkerLink, refreshOwnerSession }: OwnerScreenProps) {
  ...
  return <><PageHeading title="제가 이렇게 이해했어요" description="AI는 추측하지 않습니다. 미확정 내용은 농장주가 결정해주세요." />
    <Panel className="mx-auto max-w-4xl"><div className="text-center"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-sage text-deep"><Sparkles className="h-8 w-8" /></span><p className="mx-auto mt-6 max-w-3xl text-2xl font-black leading-10 sm:text-3xl">{draft.summary_ko}</p></div>
      ...
      <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><ActionButton variant="secondary" disabled={!draftAudio} onClick={playOriginalAudio}><Volume2 className="h-5 w-5" />원음 듣기</ActionButton><ActionButton disabled={blocked || loading || correctingQuantity || correctionPending || (needsOverride && !confirmationReason)} onClick={confirm}><Check className="h-5 w-5" />{loading ? '확정하고 있어요…' : locationAdvisoryOnly ? '현장에서 장소를 알려주고 전달' : needsOverride ? '이대로 전달' : '확정하기'}</ActionButton><ActionButton variant="quiet" onClick={() => { setDraftAudio(null); go('owner-record'); }}><Mic className="h-5 w-5" />다시 녹음</ActionButton></div>
    </Panel>
