...
밭머리는 농장주의 전라도 사투리 지시를 작업 단계로 구조화하고, 검수된 정부 가이드 표현과 사전 생성·사람 검수된 양파·딸기 작업영상을 조립해 베트남어·네팔어·캄보디아어 작업 스토리보드로 전달한다. 지시가 명확히 바뀌면 두 delivery branch의 최신 수량을 반영한다.

## P0 불변 범위

- 작물: 양파(`ONION`)와 딸기(`STRAWBERRY`) 두 vertical만 지원
- 작업 코드: 작물별 4개, 총 8개. `task_family`와 non-null `task_code`가 일치하지 않으면 invalid
- 전달 언어: `vi`, `ne`
...
P1: 농장주 회원가입/계정관리, SMS, 질문, 양파·딸기 이외 작물·추가 언어, 오프라인 캐시, 통계, 급여·출퇴근.

## 핵심 흐름

역할 선택 뒤 농장주는 오늘 작업팀 참여 QR을 생성할 수 있다. 근로자 참여 UI는 `QR 스캔 또는 참여 링크 입력 → 국적 선택 → 이름 또는 별명 입력 → 참여 요청` 순서로 진행한다. QR 인식만으로 이동하거나 팀원을 생성하지 않고, 최종 참여 성공 뒤 현재 주소를 QR의 참여 URL로 맞춘다. 국적은 베트남(`VN`)·필리핀(`PH`)·라오스(`LA`)·캄보디아(`KH`)·태국(`TH`)·네팔(`NP`)·미얀마(`MM`)·몽골(`MN`) 중 하나다. 지원 안내 언어가 확정된 베트남(`vi`)·네팔(`ne`)·캄보디아(`km`) 국적을 선택하면 다음 화면부터 해당 언어로 전환한다. 그 밖의 국적은 별도 언어 계약이 확정될 때까지 기존 안내 언어를 유지한다. 최종 참여 요청은 별명·국적·안내 언어를 함께 보낸다. 참여 정보는 오늘 작업팀 범위에만 존재하며 개인 계정을 만들지 않는다. 농장주 화면은 참여자를 별명·국적·안내 언어로 표시한다.
...
7. 역할 선택에서 농장주를 고르면 자동 데모 세션이 발급된다. owner confirm 뒤 delivery branch를 고른다. `CO_PRESENT`는 `vi|ne|km` 선택 후 demo owner session briefing에서 video+TTS를 재생한다. `REMOTE`는 `vi|ne|km` 선택 후 익명 24시간 링크를 한 번 발급한다.
