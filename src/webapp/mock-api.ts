import type { AssignmentReceipt, TeamAssignmentsResponse, Briefing, IssuedWorkerLink, LocalizedWorkerContext, OverrideReason, OwnerWorkSession, Quantity, QuantityChangePreview, TeamAssignmentMeta, TeamMember, TodayWorkTeam, Translation, V2TaskCode, V2WorkerBriefing, WorkDraft, WorkerAssignment, WorkerBadgeCode, WorkerBriefing, WorkerStep } from './contracts';
...
const receiptsFor = (member: TeamMember): AssignmentReceipt[] => member.assignment_session_ids.flatMap((sessionId) => {
  const session = loadSessions().find((item) => item.session_id === sessionId);
  if (!session) return [];
  const saved = member.assignment_receipts?.find((item) => item.work_session_id === sessionId);
  return [{ work_session_id: sessionId, current_version: session.current_version, acknowledged_version: saved?.acknowledged_version ?? null, acknowledged_at: saved?.acknowledged_at ?? null }];
});
...
export function createMockApi(ApiError: ApiErrorConstructor) {
  return {
    ...
    confirmDraft: (_draftId: string, _decision: 'CONFIRM' | 'PUBLISH_AS_IS', _reason?: OverrideReason) => wait({ work_session: saveWorkSession({ ...createSession(), session_id: loadSessions().length ? crypto.randomUUID() : 'work-demo-01' }), issued_worker_link: null }),
    ...
    createTodayTeam: (): Promise<TodayWorkTeam> => { const existing = loadTeam(); if (existing && new Date(existing.expires_at) > new Date()) { const next = { ...existing, status: 'ACTIVE' as const, join_url: `${window.location.origin}/team/team-${crypto.randomUUID().replace(/-/g, '')}` }; return wait(saveTeam(next)); } const now = new Date(); const expires = new Date(now.getTime() + 24 * 60 * 60_000); const token = `team-${crypto.randomUUID().replace(/-/g, '')}`; return wait(saveTeam({ team_id: crypto.randomUUID(), work_date: now.toLocaleDateString('sv-SE'), status: 'ACTIVE', join_url: `${window.location.origin}/team/${token}`, expires_at: expires.toISOString(), members: [] })); },
    getTodayTeam: (): Promise<TodayWorkTeam> => { const team = loadTeam(); return team ? wait({ ...team, members: team.members.map((member) => ({ ...member, assignment_receipts: receiptsFor(member) })), join_url: null }) : Promise.reject(new ApiError(404, 'NOT_FOUND', '오늘 작업팀이 아직 없습니다.')); },
    joinTodayTeam: (token: string, input: { display_name: string; language_code: WorkerLocale }): Promise<TeamMember> => { const team = loadTeam(); if (!team?.join_url?.endsWith(`/team/${token}`)) return Promise.reject(new ApiError(404, 'ACCESS_DENIED', '참여 코드를 확인해주세요.')); if (new Date(team.expires_at) <= new Date()) return Promise.reject(new ApiError(410, 'LINK_EXPIRED', '참여 시간이 끝났습니다.')); const existing = team.members.find((member) => member.member_id === (window.sessionStorage.getItem(TEAM_MEMBER_KEY) ?? window.localStorage.getItem(TEAM_MEMBER_KEY))); if (existing) return wait(existing); const member = { member_id: crypto.randomUUID(), ...input, joined_at: new Date().toISOString(), assignment_session_ids: [] }; saveTeam({ ...team, members: [...team.members, member] }); window.sessionStorage.setItem(TEAM_MEMBER_KEY, member.member_id); return wait(member); },
    ...
    getMyTeamAssignments: (): Promise<TeamAssignmentsResponse> => {
      const team = loadTeam();
      const member = team?.members.find((item) => item.member_id === (window.sessionStorage.getItem(TEAM_MEMBER_KEY) ?? window.localStorage.getItem(TEAM_MEMBER_KEY)));
      if (!team || new Date(team.expires_at).getTime() <= Date.now()) return Promise.reject(new ApiError(410, 'LINK_EXPIRED', '접근할 수 없습니다.'));
      if (!member) return wait({ assignments: [], receipts: [] });
      const assignments = member.assignment_session_ids.flatMap((sessionId) => { const session = loadSessions().find((item) => item.session_id === sessionId); return session ? [v2Briefing(session, member.language_code, session.version.state.task_family)] : []; });
      return wait({ assignments, receipts: receiptsFor(member) });
    },
    acknowledgeAssignment: (sessionId: string, expectedVersion: number): Promise<AssignmentReceipt> => {
      const team = loadTeam();
      const member = team?.members.find((item) => item.member_id === (window.sessionStorage.getItem(TEAM_MEMBER_KEY) ?? window.localStorage.getItem(TEAM_MEMBER_KEY)));
      if (!team || new Date(team.expires_at).getTime() <= Date.now()) return Promise.reject(new ApiError(410, 'LINK_EXPIRED', '접근할 수 없습니다.'));
      const receipt = member && receiptsFor(member).find((item) => item.work_session_id === sessionId);
      if (!member || !receipt) return Promise.reject(new ApiError(404, 'ACCESS_DENIED', '접근할 수 없습니다.'));
      if (receipt.current_version !== expectedVersion) return Promise.reject(new ApiError(409, 'VERSION_CONFLICT', '최신 지시를 다시 확인하세요.'));
      const next = { ...receipt, acknowledged_version: expectedVersion, acknowledged_at: receipt.acknowledged_version === expectedVersion ? receipt.acknowledged_at : new Date().toISOString() };
      saveTeam({ ...team, members: team.members.map((item) => item.member_id === member.member_id ? { ...item, assignment_receipts: [...(item.assignment_receipts ?? []).filter((saved) => saved.work_session_id !== sessionId), next] } : item) });
      return wait(next);
    },
    getMyTodayAssignments: (): Promise<WorkerAssignment[]> => { const memberId = (window.sessionStorage.getItem(TEAM_MEMBER_KEY) ?? window.localStorage.getItem(TEAM_MEMBER_KEY)); const team = loadTeam(); const member = team?.members.find((item) => item.member_id === memberId); if (!member) return wait([]); return wait(member.assignment_session_ids.flatMap((sessionId) => { const session = loadSessions().find((item) => item.session_id === sessionId); return session ? [v2Briefing(session, member.language_code, session.version.state.task_family)] : []; })); },
  };
}
